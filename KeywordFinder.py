...
def testing():
    ...
    human_keywords_test1 = ['pytorch', 'dgl', 'networkx', 'xgboost', 'lightgbm', 'dvc', 'nvidia', 'nemo', 'huggingface',
                            ...
                            'aws', 'postgres', 'elasticsearch', 'sqlite', 's3', 'research', 'ml', 'training', 'model',
                            'modeling',
                            'partner', 'bachelor', 'intern', 'data',
                            'python', 'pytorch', 'tensorflow', 'statistical', 'coding', 'documentation',
                            'communication',
    ...
    human_keywords_test2 = ['solutions', 'build', 'sales', 'opportunities', 'teams', 'data', 'internal', 'external',
                            'simplify',
                            'automate', 'workflows', 'business', 'pipelines', 'metrics', 'key',
                            'hadoop', 'spark', 'hive', 'kafka', 'structured', 'unstructured', 'dataset', 'dbms',
                            'rdbms', 'etl',
                            'sql', 'design', 'python', 'pyspark', 'scala', 'warehousing', 'modeling', 'problem-solving',
                            'trouble-shooting',
                            'airflow', 'argo', 'computer', 'science', 'hdfs', 'informatica', 'mpp', 'qliksense',
                            'tableau',
    ...
    cosine_likeness_score = len(list(set(human_keywords_test1).intersection(keywords_cosine))) / len(
        human_keywords_test1)
    kernels_likeness_score = len(list(set(human_keywords_test1).intersection(keywords_kernels))) / len(
        human_keywords_test1)
    laplacian_likeness_score = len(list(set(human_keywords_test1).intersection(keywords_laplacian))) / len(
        human_keywords_test1)
    ...
    cosine_likeness_score2 = len(list(set(human_keywords_test2).intersection(keywords_cosine2))) / len(
        human_keywords_test2)
    kernels_likeness_score2 = len(list(set(human_keywords_test2).intersection(keywords_kernels2))) / len(
        human_keywords_test2)
    laplacian_likeness_score2 = len(list(set(human_keywords_test2).intersection(keywords_laplacian2))) / len(
        human_keywords_test2)
...
def get_keywords(text):
    ...
    candidates = counts.get_feature_names_out()
